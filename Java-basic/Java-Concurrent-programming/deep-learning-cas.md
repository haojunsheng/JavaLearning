...
![img](https://ws3.sinaimg.cn/large/006tKfTcly1g0n1u7en4lj30hb0cdwei.jpg)
...
CAS需要检查操作值有没有发生改变，如果没有发生改变则更新。但是存在这样一种情况：如果一个值原来是A，变成了B，然后又变成了A，那么在CAS检查的时候会发现没有改变，但是实质上它已经发生了改变，这就是所谓的ABA问题。对于ABA问题其解决方案是加上版本号，即在每个变量都加上一个版本号，每次改变时加1，即A —> B —> A，变成1A —> 2B —> 3A。（**对于纯数值是没有任何影响的**）

用一个例子来阐述ABA问题所带来的影响。

[ABA问题实例](https://blog.csdn.net/bjweimengshu/article/details/79000506)

CAS的ABA隐患问题，解决方案则是**版本号** ，Java提供了AtomicStampedReference来解决。AtomicStampedReference通过包装[E,Integer]的元组来对对象标记版本戳stamp，从而避免ABA问题。对于上面的案例应该线程1会失败。
...
![img](https://ws3.sinaimg.cn/large/006tKfTcly1g0n4n2cv6oj30do03v74l.jpg)
